...
network:
	  docker network create devstart || true

cs-fix:
	docker exec devstart-app ./vendor/bin/pint

composer-install:
	docker exec devstart-app composer install

cp-env:
	docker exec devstart-app cp .env.example .env

artisan-key-generate:
	docker exec devstart-app php artisan key:generate

artisan-migrate:
	docker exec devstart-app php artisan migrate
